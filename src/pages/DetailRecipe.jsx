...
import { FaHeart, FaHeartBroken, FaRegHeart, FaSignInAlt } from 'react-icons/fa';
import { useFavorites } from '../context/FavoriteContext';
import { isFavorite } from '../api/favoriteService';
    ...
    useEffect(() => {
        const loadRecipe = async () => {
            ...
            try {
                ...
                setError(err);
                console.error('Error obteniendo:', err);
            } finally {
                setLoading(false);
            }
        };
                ...
                <div className="row g-0">
                    ...
                    <div className="col-lg-5">
                        <div className="position-relative h-100">
                            <img
                                src={recipe.image || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxleHBsb3JlLWZlZWR8MXx8fGVufDB8fHx8&w=1000&q=80'}
                                alt={recipe.title}
                                    ...
                                    <div className="d-flex gap-2">
                                        ...
                                        {!user && (
                                            <button
                                                ...
                                                onClick={() => navigate('/login', { state: { from: `/recipes/${recipe.id}` } })}
                                        ...
                                        {isFavorite ? (
                                            <button onClick={toggleFavorite} className="btn btn-sm btn-outline-danger">
                                                <FaHeart className="me-1" />
                                                <span className="d-none d-sm-inline">Eliminar de favoritos</span>
                                            </button>
                                        ) : (
                                            <button onClick={toggleFavorite} className="btn btn-sm btn-outline-primary">
                                                <FaRegHeart className="me-1" />
                                                <span className="d-none d-sm-inline">Agregar a favoritos</span>
                                            </button>
                                        )}
                                    </div>
                                )}
                    ...
                    <div className="col-lg-7">
                        <div className="card-body p-4">
                            <h1 className="card-title fw-bold mb-3 text-primary">{recipe.title}</h1>
                                ...
                                <div className="bg-light p-3 rounded">
                                    {recipe.preparation.split('\n').map((step, i) => (
                                        <p key={i} className="mb-2">
                                            <strong className="text-primary">Paso {i + 1}:</strong> {step}
                                        </p>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="mb-4">
                                ...
                                <ul className="list-group">
                                    {recipe.ingredients?.length > 0 ? (
                                        ...
                                        <li className="list-group-item text-muted">No hay ingredientes</li>
                                    )}
                                </ul>
                            </div>
